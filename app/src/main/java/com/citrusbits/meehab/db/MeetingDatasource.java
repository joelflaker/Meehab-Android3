...
public class MeetingDatasource {
	...
	public UserAccount add(UserAccount user) {
		...
		values.put(MeehabDBOpenHelper.COLUMN_INTRESTED_IN,user.getIntrestedIn());
		values.put(MeehabDBOpenHelper.COLUMN_ABOUT_STORY,user.getAboutStory());
		values.put(MeehabDBOpenHelper.COLUMN_WILLING_SPONSOR,user.getWillingSponsor());
	...
	public List<UserAccount> findAll() {
		...
		if (c.getCount() > 0) {
			while (c.moveToNext()) {
				...
				user.setId(c.getInt(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_USER_ID)));
				user.setUsername(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_USER_NAME)));
				user.setEmail(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_USER_EMAIL)));
				user.setPhone(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_PHONE)));
				user.setImage(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_PICTURE)));
				user.setGender(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_GENDER)));
				user.setDateOfBirth(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_DOB)));
				user.setMeetingHomeGroup(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_HOME_GROUP)));
				user.setMaritalStatus(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_MARITAL_STATUS)));
				user.setIntrestedIn(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_INTRESTED_IN)));
				user.setAboutStory(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_ABOUT_STORY)));
				user.setWillingSponsor(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_WILLING_SPONSOR)));
				user.setHeight(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_HEIGHT)));
				user.setWeight(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_WEIGHT)));
				user.setSexualOrientation(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_SEXUAL_ORIENTATION)));
				user.setEthnicity(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_ETHNICITY)));
				user.setLatitude(c.getInt(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_LAT)));
				user.setLongitude(c.getInt(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_LNG)));
				user.setSoberSence(c.getString(c.getColumnIndex(MeehabDBOpenHelper.COLUMN_SOBER_DATE)));
